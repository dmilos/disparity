...
StereoPatchMatch::StereoPatchMatch()
 {
  // preset 1
  m_trash = 0.1;
  m_side = 6;
  m_square.m_trashold = 0.09;
  m_square.m_proximity = 0.0;

  // preset 2
  m_trash = 1;
  m_side = 6;
  m_square.m_trashold = 0.03;
  m_square.m_proximity = 0.0009;
 }

void StereoPatchMatch::process( ImageType & disparity, ImageType const& left, ImageType const& right )const
 {
...
void StereoPatchMatch::process_P(  SizeType const& line, ImageType const& left, ImageType const& right )const
{
  int width = (int)left.cols;
  int height = (int)right.cols;
...
void StereoPatchMatch::processL( ImageType & disparity, SizeType const & line, ImageType const& left, ImageType const& right )const
 {
  m_square.processL(  );
  m_square.disparityL();
...
void StereoPatchMatch::processR( ImageType& disparity, SizeType const& line, ImageType const& left, ImageType const& right )const
 {
  m_square.processR();
  m_square.disparityR();
...
void StereoPatchMatch::processB( ImageType& disparity, SizeType const& line, ImageType const& left, ImageType const& right )const
 {
  m_square.disparityB();
...
StereoPatchMatch::ScalarType StereoPatchMatch::distance( Size2DType const &L, Size2DType const &R, ImageType const& left, ImageType const& right )const
 {
  ...
  auto result = summae / counter;

  return result;
 }

StereoPatchMatch::ScalarType StereoPatchMatch::distance( ColorType const &left, ColorType const & right )const
 {
  //color::gray<double> L; L = left;
  //color::gray<double> R; R = right;
  //color::lab<double> L; L = left;
  //color::lab<double> R; R = right;  
  //auto d = ::color::operation::distance( L, R )/100.0 ;
  //return d;
...
StereoPatchMatch::ColorType StereoPatchMatch::get( Size2DType const location, ImageType const image )const
 {
  ColorType result;
